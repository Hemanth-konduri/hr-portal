...
import { DollarSign, TrendingDown, CalendarDays, Eye, RefreshCw } from 'lucide-react'
...
export default function EmployeePayslipsPage() {
  ...
  const filtered = payslips.filter(p => yearFilter === 'all' || String(p.year) === yearFilter)
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-3">
          <StatCard title="Latest Net Salary"
          ...
          <StatCard title="Total Earned (filtered)"
            value={`₹${totalNet.toLocaleString()}`}
            icon={DollarSign} iconClass="bg-blue-50 text-blue-600"
            sub={`${filtered.length} payslip${filtered.length !== 1 ? 's' : ''}`} />
          <StatCard title="Total LOP Days"
            value={totalLop}
            icon={TrendingDown} iconClass="bg-red-50 text-red-500"
            sub="Loss of pay days (filtered)" />
        </div>
      )}
          ...
          {selected && (
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center rounded-lg bg-emerald-50 px-4 py-3">
                ...
                <span className="text-xl font-bold text-emerald-700">₹{Number(selected.net_salary).toLocaleString()}</span>
              </div>
            </div>
          )}
        </DialogContent>
